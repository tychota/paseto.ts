...
describe('rsa_keygen_addon', () => {
  it('true', () => {
    expect(true).toBe(true);
  });
  describe('generateRsaPrivateKey', () => {
    it('should generate a pem encoded key', async () => {
      const rawPrivateKey = await generateRsaPrivateKey();
      expect(typeof rawPrivateKey === 'string').toBeTruthy();
      expect('-----BEGIN RSA PRIVATE KEY-----').toBe(rawPrivateKey.slice(0, 31));
    });
  });
  describe('extractRsaPublicKey', () => {
    const rawPrivateKey = `-----BEGIN RSA PRIVATE KEY-----
      ...
      -----END RSA PRIVATE KEY-----\n`;
    const expectedRawPublicKey = `-----BEGIN RSA PUBLIC KEY-----
      ...
      -----END RSA PUBLIC KEY-----\n`;
    it('should extract a public key from secret key synchronously', async () => {
      const rawPublicKey = await extractRsaPublicKey(rawPrivateKey);
      expect(typeof rawPublicKey === 'string').toBeTruthy();
      expect(rawPublicKey).toBe(expectedRawPublicKey);
    });
  });
});
